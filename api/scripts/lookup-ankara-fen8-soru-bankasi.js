// Teşhis: Ankara Yayıncılık "Fen Bilimleri Güçlendiren Soru Bankası" 8 kaynağını bulur.
// Usage: node api/scripts/lookup-ankara-fen8-soru-bankasi.js
const fs = require('fs')
...
async function main() {
  ...
  try {
    const books = await pool.request().query(`
      ...
      WHERE b.name LIKE '%Güçlendiren%' OR b.name LIKE '%Fen Bilimleri%Soru Bankas%' OR p.name LIKE '%Ankara%'
      ORDER BY b.created_at DESC;`)
    console.log('Eşleşen kitaplar:', JSON.stringify(books.recordset, null, 2))
