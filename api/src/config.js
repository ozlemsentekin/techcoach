...
function getRuntimeConfig() {
  ...
  return {
    ...
    // Set-Cookie'de sabit bir Domain yazmak için. DİKKAT: Azure SWA arkasında BOŞ
    // bırakılmalı — uygulama Set-Cookie'ye kendi Domain'ini eklediğinde SWA proxy'si
    // başlığı komple düşürüyor (login/logout çerez basamıyor). SWA zaten isteğin
    // geldiği host'u Domain olarak enjekte ediyor; apex+www mükerrer çerez sorunu
    // index.html'deki www→apex redirect ile çözülüyor (bkz. PR #71). Bu knob yalnız
    // SWA dışı bir barındırma senaryosu için var.
...
function getBillingConfig() {
  ...
  return {
    ...
    // Öğretmenin sabit panel ücretine (2999 TL/ay, 4 öğrenci dahil) ek olarak satın aldığı
...
function getIyzicoConfig() {
  return {
    ...
    // Öğretmenin ek öğrenci koltuğu (öğrenci başı aylık 299 TL / yıllık 4.990 TL) paketi.
    ...
    teacherSeatMonthlyPlanRef: getOptionalEnv('IYZICO_TEACHER_SEAT_MONTHLY_PLAN_REF', {
      placeholders: ['replace-with-iyzico-teacher-seat-monthly-plan-ref'],
    }),
    teacherSeatYearlyPlanRef: getOptionalEnv('IYZICO_TEACHER_SEAT_YEARLY_PLAN_REF', {
      placeholders: ['replace-with-iyzico-teacher-seat-yearly-plan-ref'],
    }),
