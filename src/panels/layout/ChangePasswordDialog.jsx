...
// Giriş ekranı yalnızca 6 haneli rakamlı şifre kabul ettiği için (pattern="[0-9]*"),
// burada belirlenen yeni şifre de tam olarak 6 rakamdan oluşmalı.
const onlyDigits = (value) => String(value || '').replace(/\D/g, '').slice(0, PASSWORD_LENGTH)
...
export default function ChangePasswordDialog({ onClose, required = false, onSaved, onLogout }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (newPassword.length !== PASSWORD_LENGTH) {
      setError('Yeni şifre tam olarak 6 rakamdan oluşmalı.')
      return
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center overflow-hidden bg-black/30 p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true">
      ...
      >
        ...
        {required ? <p className="mt-4 text-sm leading-6 text-panel-text-muted">Başlangıç şifreniz, bu hesaba kayıtlı telefon numarasının son 6 hanesidir. Panele devam etmek için yalnızca sizin bildiğiniz yeni bir şifre belirleyin. Yeni şifre tam olarak 6 rakamdan oluşmalı ve başlangıç şifresinden farklı olmalıdır.</p> : null}
