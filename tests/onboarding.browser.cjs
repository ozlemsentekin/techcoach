...
;(async () => {
  ...
  try {
    ...
    await page.getByText('İpek için profiliniz hazır.', { exact: false }).waitFor()
    assert.equal(await page.locator('[role="dialog"]').count(), 0)
    assert.equal(await page.getByRole('link', { name: 'Çocuk profili oluştur', exact: true }).count(), 0)
    assert.equal(await page.getByRole('navigation', { name: 'Kullanım yol haritası' }).getByRole('button').count(), 4)
    assert.equal(await page.getByRole('region', { name: 'Seçilen rehber adımı' }).count(), 1)
    ...
    console.log('PASS existing child, new parent, menu/welcome entry points, four phases and single step, mobile overflow, profile/library links, direct route reload')
