...
;(async () => {
  ...
  try {
    ...
    await page.reload()
    await page.getByRole('heading', { name: 'İlk giriş: şifrenizi belirleyin' }).waitFor()
    await page.getByLabel('Başlangıç şifresi (telefonun son 6 hanesi)', { exact: true }).fill('111111')
    await page.getByLabel('Yeni şifre', { exact: true }).fill('654321')
    await page.getByLabel('Yeni şifre (tekrar)', { exact: true }).fill('654321')
